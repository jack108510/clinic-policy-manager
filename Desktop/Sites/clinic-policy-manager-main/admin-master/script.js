...
(function attachFunctionsImmediately() {
    ...
    try {
        ...
        window.toggleCompanyMenu = toggleCompanyMenu;
...
// Company Management - Redesigned with card layout
function displayCompanies() {
    ...
    const filteredCompanies = companies.filter(company => {
        const matchesSearch = !searchTerm || 
            ...
            (company.accessCode && company.accessCode.toLowerCase().includes(searchTerm));
    ...
    filteredCompanies.forEach(company => {
        ...
        companyCard.className = `company-card ${company.status === 'suspended' ? 'suspended' : ''}`;
        companyCard.innerHTML = `
            <div class="company-card-header">
                ...
                <div class="company-actions-dropdown">
                    <button class="btn-icon" onclick="event.stopPropagation(); toggleCompanyMenu('${company.id}')">
                        <i class="fas fa-ellipsis-v"></i>
                    </button>
                    <div class="dropdown-menu" id="menu-${company.id}" style="display: none;">
                        <button onclick="showCompanyDetails('${company.id}')">
                            <i class="fas fa-eye"></i> View Details
                        </button>
                        <button onclick="showCompanyDetails('${company.id}', 'webhooks')">
                            <i class="fas fa-plug"></i> Manage Webhooks
                        </button>
                        <button onclick="configureCompanyAPI('${company.id}')">
                            <i class="fas fa-cog"></i> Configure API
                        </button>
                        <button onclick="suspendCompany('${company.id}')" class="${company.status === 'active' ? '' : 'text-success'}">
                            <i class="fas fa-${company.status === 'active' ? 'ban' : 'check'}"></i> ${company.status === 'active' ? 'Suspend' : 'Reactivate'}
                        </button>
                        <button onclick="deleteCompany('${company.id}')" class="text-danger">
            ...
            <div class="company-card-body">
                <div class="company-stats-grid">
                    <div class="stat-item">
                        ...
                        <div class="stat-content">
                            <div class="stat-value">${company.users || 0}</div>
                    ...
                    <div class="stat-item">
                        ...
                        <div class="stat-content">
                            <div class="stat-value">${company.policies || 0}</div>
                ...
                <div class="company-info-section">
                    <div class="info-row">
                        <span class="info-label"><i class="fas fa-envelope"></i> Admin Email:</span>
                        <span class="info-value">${company.adminEmail || 'Not set'}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label"><i class="fas fa-key"></i> Access Code:</span>
                        <span class="info-value">${company.accessCode || 'N/A'}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label"><i class="fas fa-lock"></i> Admin Password:</span>
                        <div class="info-value password-group">
                            <span class="password-display">${company.adminPassword ? '••••••••' : 'Not Set'}</span>
                            ${company.adminPassword ? `
                                <button onclick="copyToClipboard('${company.adminPassword}', 'Admin Password')" class="btn-icon-small" title="Copy password">
                                    <i class="fas fa-copy"></i>
                                </button>
                            ` : ''}
                            <button onclick="setCompanyPassword('${company.id}')" class="btn-icon-small" title="Set password">
                ...
                <div class="company-config-section">
                    <div class="config-item">
                        ...
                        <div class="config-status">
                            <span class="status-chip ${webhookStatusClass}">${webhookStatusLabel}</span>
                            <button onclick="showCompanyDetails('${company.id}', 'webhooks')" class="btn-link">Manage</button>
                        </div>
                    </div>
                    <div class="config-item">
                        <div class="config-label">
                        ...
                        <div class="config-status">
                            <span class="status-chip ${company.apiKey ? 'configured' : 'not-configured'}">
                                ${company.apiKey ? 'Configured' : 'Not Set'}
                            </span>
                            <button onclick="configureCompanyAPI('${company.id}')" class="btn-link">Configure</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="company-card-footer">
                <div class="footer-info">
                    <span><i class="fas fa-calendar"></i> Created: ${formatDate(company.signupDate)}</span>
                    <span><i class="fas fa-clock"></i> Last Active: ${formatDate(company.lastActive)}</span>
                </div>
                <div class="footer-actions">
                    <button onclick="showCompanyDetails('${company.id}')" class="btn btn-small btn-primary">
                        <i class="fas fa-eye"></i> View
                    </button>
                    <button onclick="deleteCompany('${company.id}')" class="btn btn-small btn-danger">
...
function toggleCompanyMenu(companyId) {
    const menu = document.getElementById(`menu-${companyId}`);
    // Close all other menus
    document.querySelectorAll('.dropdown-menu').forEach(m => {
        if (m.id !== `menu-${companyId}`) {
...
function deleteUser(userId) {
    ...
    if (confirm(`Are you sure you want to delete user "${user.username}" (${user.email}) from ${user.company}? This action cannot be undone.`)) {
        ...
        if (userIndex !== -1) {
            ...
            // Send deletion data to webhook BEFORE removing from array
            ...
            if (webhookFn) {
                ...
                webhookFn(deletionData, 'userdeleted').catch(error => {
                    console.error('Error sending user deletion to webhook:', error);
                    // Continue with deletion even if webhook fails
                });
                console.log('✅ User deletion data sent to webhook:', deletedUser.username);
            } else {
...
function suspendCompany(companyId) {
    ...
    const confirmMessage = company.status === 'active' 
        ? `Are you sure you want to suspend company "${company.name}"?`
        : `Are you sure you want to reactivate company "${company.name}"?`;
    
    if (confirm(confirmMessage)) {
        const wasActive = company.status === 'active';
        company.status = company.status === 'active' ? 'suspended' : 'active';
        
        // Send suspension/reactivation data to webhook
        const webhookFn = typeof sendToWebhook !== 'undefined' ? sendToWebhook : (typeof window !== 'undefined' && window.sendToWebhook ? window.sendToWebhook : null);
        
        if (webhookFn && wasActive) {
            // If suspending, send to webhook
            const suspensionData = {
                ...company,
                suspended_at: new Date().toISOString(),
                suspended_by: 'admin-master',
                action: 'suspended'
            };
            webhookFn(suspensionData, 'companydeleted').catch(error => {
                console.error('Error sending company suspension to webhook:', error);
                // Continue with suspension even if webhook fails
            });
        }
        ...
        showAlert(`Company ${action === 'suspend' ? 'suspended' : 'reactivated'} successfully!`, 'success');
    }
}
...
function deleteCompany(companyId) {
    ...
    const confirmMessage = `⚠️ WARNING: Are you sure you want to PERMANENTLY DELETE company "${company.name}"?\n\nThis will:\n• Delete the company and all associated data\n• Remove all users from this company\n• Delete all policies\n• This action CANNOT be undone!\n\nType "${company.name}" to confirm deletion:`;
    
    // Use a more secure confirmation
    ...
    if (userInput === company.name) {
        ...
        if (typeof SupabaseDB !== 'undefined' && SupabaseDB.deleteCompany) {
            SupabaseDB.deleteCompany(companyId).catch(error => {
        ...
        // Also remove users associated with this company
        users = users.filter(u => u.company !== company.name);
        localStorage.setItem('masterUsers', JSON.stringify(users));
        ...
        // Add to activity feed
        addActivity(`Deleted company: ${company.name}`);
        
        showAlert(`Company "${company.name}" has been deleted successfully.`, 'success');
    } else if (userInput !== null) {
        // User typed something but it was wrong
        showAlert('Company name did not match. Deletion cancelled.', 'warning');
    }
    // If userInput is null, user cancelled the prompt
}
...
function bulkDeleteUsers() {
    ...
    if (confirm(`Are you sure you want to delete ${selectedUsers.length} user(s): ${userNames}? This action cannot be undone.`)) {
        // Send deletion data to webhook for each user BEFORE deleting
        const webhookFn = typeof sendToWebhook !== 'undefined' ? sendToWebhook : (typeof window !== 'undefined' && window.sendToWebhook ? window.sendToWebhook : null);
        ...
        if (webhookFn) {
            selectedUsers.forEach(async (user) => {
                ...
                try {
                    await webhookFn(deletionData, 'userdeleted');
                    console.log('✅ User deletion sent to webhook:', user.username);
                } catch (error) {
                    console.error('Error sending user deletion to webhook:', error);
                    // Continue with deletion even if webhook fails
                }
...
function showCreateUserModal() {
    ...
    if (companySelect) {
        companySelect.innerHTML = '<option value="">No Company (Standalone User)</option>';
        
        // Load companies if not already loaded
        if (!companies || companies.length === 0) {
            const localCompanies = JSON.parse(localStorage.getItem('masterCompanies') || '[]');
...
async function createUserFromAdmin(event) {
    ...
    const existingUsers = JSON.parse(localStorage.getItem('masterUsers') || '[]');
    const usernameExists = existingUsers.some(u => u.username && u.username.toLowerCase() === username.toLowerCase());
    ...
    // Send to webhook via SupabaseDB.createUser
    if (typeof SupabaseDB !== 'undefined' && SupabaseDB.createUser) {
        try {
            await SupabaseDB.createUser(newUser);
            console.log('✅ User created and sent to webhook:', username);
        } catch (error) {
            console.error('Error creating user via webhook:', error);
            showAlert('User created locally but webhook failed. User may not be synced.', 'warning');
        }
    } else {
        ...
        if (webhookFn) {
            try {
                await webhookFn(newUser, 'user');
                console.log('✅ User sent to webhook directly:', username);
            } catch (error) {
        ...
        localStorage.setItem('masterUsers', JSON.stringify(existingUsers));
    }
    
    // Update local users array
    users = JSON.parse(localStorage.getItem('masterUsers') || '[]');
    
    // Update company user count if company is assigned
    if (company) {
        const companyObj = companies.find(c => c.name === company);
        if (companyObj) {
            companyObj.users = (companyObj.users || 0) + 1;
            localStorage.setItem('masterCompanies', JSON.stringify(companies));
        }
    }
    ...
    // Add to activity feed
    addActivity(`Created new user: ${username}${company ? ` (${company})` : ''}`);
}
...
async function createAccessCode(event) {
    ...
    try {
        ...
        // If called from company modal, update the access code dropdown
        const companyModalAccessCode = document.getElementById('accessCode');
        const companyModal = document.getElementById('launchCompanyModal');
        if (companyModalAccessCode && companyModal) {
            // Refresh the access code dropdown
            ...
            companyModalAccessCode.innerHTML = '<option value="">Select Access Code</option>';
            activeCodes.forEach(code => {
                ...
                if (code.code === normalizedCode.code) {
                    option.selected = true;
                }
                companyModalAccessCode.appendChild(option);
            });
        }
        ...
        // Reopen company modal if it was open before
        if (companyModal && document.getElementById('createCodeModal').dataset.fromCompanyModal === 'true') {
            companyModal.classList.add('show');
            delete document.getElementById('createCodeModal').dataset.fromCompanyModal;
        }
...
async function deleteAccessCode(codeId) {
    if (confirm('Are you sure you want to delete this access code?')) {
        // Find the access code before deletion
...
function launchNewCompany() {
    ...
    if (!userSelect) {
        console.error('❌ adminUserSelect element not found!');
        showAlert('Error: User dropdown not found', 'error');
        return;
    }
    ...
    if (users.length === 0) {
        ...
        noUsersOption.value = '';
        noUsersOption.textContent = 'No users available - create a user first';
        ...
        users.forEach(user => {
            ...
            const displayName = user.full_name || user.fullName || user.username || user.email || 'Unknown User';
            const email = user.email ? ` (${user.email})` : '';
            option.textContent = `${displayName}${email}`;
            userSelect.appendChild(option);
        });
        console.log('✅ Populated user dropdown with', users.length, 'users');
    }
...
// Function to create access code from company modal
function createAccessCodeFromCompanyModal(event) {
    event.preventDefault();
    event.stopPropagation();
    
    // Close company modal temporarily
    const companyModal = document.getElementById('launchCompanyModal');
    const wasOpen = companyModal.classList.contains('show');
    if (wasOpen) {
    ...
    // Store a flag to know we came from company modal
    const createCodeModal = document.getElementById('createCodeModal');
    if (createCodeModal) {
        createCodeModal.dataset.fromCompanyModal = 'true';
    }
}
if (typeof window !== 'undefined') window.createAccessCodeFromCompanyModal = createAccessCodeFromCompanyModal;
...
async function launchCompany(event) {
    ...
    if (!selectedUserId) {
        showAlert('Please select a user to grant admin privileges', 'error');
        return;
    }
    ...
    if (!selectedAccessCode) {
        showAlert('Please select or create an access code', 'error');
        return;
    }
    ...
    if (!selectedUser) {
        showAlert('Selected user not found!', 'error');
        return;
    }
    ...
    const newCompany = {
        ...
        adminName: selectedUser.full_name || selectedUser.fullName || selectedUser.username,
    ...
    // Send company to webhook
    if (typeof SupabaseDB !== 'undefined' && SupabaseDB.createCompany) {
        try {
            await SupabaseDB.createCompany(newCompany);
        } catch (error) {
            console.error('Error creating company via webhook:', error);
        }
    }
    
    // Add company to local storage
    companies.push(newCompany);
    localStorage.setItem('masterCompanies', JSON.stringify(companies));
    
    // Grant admin privileges to selected user
    selectedUser.isAdmin = true;
    selectedUser.company = companyName;
    selectedUser.company_id = newCompany.id;
    selectedUser.role = 'Admin';
    selectedUser.password = adminPassword; // Set the admin password for the user
    
    // Update user in localStorage
    const userIndex = users.findIndex(u => u.id === selectedUserId);
    if (userIndex !== -1) {
        users[userIndex] = { ...users[userIndex], ...selectedUser, password: adminPassword };
    } else {
        users.push(selectedUser);
    }
    
    // Save user to Supabase if configured, otherwise localStorage
    if (typeof SupabaseDB !== 'undefined' && SupabaseDB.updateUser) {
        try {
            await SupabaseDB.updateUser(selectedUserId, {
                isAdmin: true,
                company: companyName,
                company_id: newCompany.id,
                role: 'Admin',
                password: adminPassword // Include password in update
            });
        } catch (error) {
            console.warn('⚠️ Failed to update user, using localStorage:', error);
        }
    }
    
    // Always save to localStorage
    localStorage.setItem('masterUsers', JSON.stringify(users));
    console.log('✅ Admin privileges granted to user:', selectedUser.username);
    ...
    showAlert(`Company "${newCompany.name}" launched successfully! ${selectedUser.username} has been granted admin privileges.`, 'success');
}
